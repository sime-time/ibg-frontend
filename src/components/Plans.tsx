...
const plans: PlanProps[] = [
  ...
  },
  {
    ...
    price: 110,
    features: ["Unlimited group classes", "Access to 1-on-1 training", "Free wellness consultation"],
    popular: true,
  },
  {
    id: "mma",
...
export default function Plans() {
  return (
    <section id="plans" class="py-16 px-1 md:px-16 flex flex-col justify-center items-center gap-0 md:gap-7">
      <div role="tablist" class="tabs tabs-md tabs-boxed bg-gray-100 bg-opacity-10 border border-gray-400/40 rounded-xl w-fit md:w-1/3 ">
        <button role="tab" onClick={() => setShowYearly(false)} class={`tab ${showYearly() ? "" : "tab-active"}`} >Monthly</button>
        <button role="tab" onClick={() => setShowYearly(true)} class={`tab flex gap-2 ${showYearly() ? "tab-active" : ""}`}>Yearly <span class="badge badge-success">16% off</span></button>
      </div>
    ...
    <div id={props.id} class={`flex flex-col justify-between gap-9 p-9 rounded-xl ${props.popular ? "bg-gray-100 bg-opacity-10 border border-gray-400/40" : ""}`}>
      ...
      <div class="flex flex-col gap-4">
        <div class="flex items-center gap-2">
          <h3 class="text-4xl font-bold">{new Intl.NumberFormat("en-US", {
            style: "currency",
            currency: "USD",
            trailingZeroDisplay: "stripIfInteger"
          }).format(props.price)}</h3>
          <span class="text-sm text-gray-400 leading-none">per<br />{showYearly() ? "year" : "month"}</span>
        </div>
        <div class="flex flex-col gap-3">
          ...
          <ul class="text-sm opacity-80 leading-loose">
            ...
            <For each={props.features}>
              {(feature) => (
                <li class="flex gap-3 items-center"><FaSolidCircleCheck />{feature}</li>
              )}
            </For>
          </ul>
        </div>
